...
Nodes indicate their \gloss{commmitment} to providing a service by signing against the game contract either in a transaction or issuing a signed note offchain. 
...
%
An entry is created if provider signs off on a service game contract in an act of \gloss{commitment}. Essentially, the swear contract only needs to record how much deposit each commitment promised and let the swap channel handle the deposit.
